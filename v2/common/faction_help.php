...
function AddFaction($chatpath, $new_name) {
	...
	$new_name = preg_replace("@([^()-/´`~={}><*:#&[:alnum:][:space:]]+)@i", "", $new_name);

	$result = @mysqli_query($handler, "SELECT name FROM uo_chat_faction WHERE name='$new_name' AND chat='$chatpath'");
	$row = @mysqli_fetch_row($result);
	@mysqli_free_result($result);

	if ($row[0] != $new_name) {
		@mysqli_query($handler, "INSERT INTO uo_chat_faction (chat,name) VALUES ('$chatpath','$new_name')");
...
function RenameFaction($chatpath, $old_id, $new_name) {
	...
	if ($row[0] != $new_name) {
		@mysqli_query($handler, "UPDATE uo_chat_faction SET name='$new_name' WHERE id='$old_id' AND chat='$chatpath'");
...
function DeleteFaction($chatpath, $old_id) {
	...
	@mysqli_query($handler, "DELETE FROM uo_chat_faction WHERE id='$old_id' AND chat='$chatpath'");
	@mysqli_query($handler, "UPDATE uo_chat_database SET faction='0' WHERE faction='$old_id' AND chat='$chatpath'");
...
function ListFactions($chatpath) {
	...
	$result = @mysqli_query($handler, "SELECT name,id FROM uo_chat_faction WHERE chat='$chatpath' ORDER BY id ASC");
...
function GetFactionNames($chatpath) {
	...
	$result = @mysqli_query($handler, "SELECT id,name FROM uo_chat_faction WHERE chat='$chatpath'");
...
function GetFactionIcon($chatpath, $faction) {
	...
	if (mysqli_num_rows($result) && $row = mysqli_fetch_row($result)) {
		return $row[0];
	}
	mysqli_free_result($result);
